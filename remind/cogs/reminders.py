...
GuildSettings = recordtype(
    ...
    [
        ...
        ("remind_role_id", None),
        ("remind_before", None),
        ("finalcall_channel_id", None),
        ("finalcall_before", None),
...
async def _send_reminder_at(request):
    ...
    if request.contest.is_rare():
        embed.set_footer(
            text=f"Its once in a while contest, you wouldn't wanna miss 👀"
        )
    for website, name, value in _get_embed_fields_from_contests([request.contest]):
        embed.add_field(
            name=_get_display_name(website, name), value=value, inline=False
        )
    await request.channel.send(
        request.role.mention + f" Its {website} time!", embed=embed
    )
...
class Reminders(commands.Cog):
        ...
        try:
            with guild_map_path.open("rb") as guild_map_file:
                ...
                for guild_id, guild_settings in guild_map.items():
                    self.guild_map[guild_id] = GuildSettings(
                        **{
                            key: value
                            for key, value in guild_settings._asdict().items()
    ...
    async def _update_task(self):
        ...
        self.finished_contests.sort(
            key=lambda contest: contest.start_time + contest.duration, reverse=True
        )
        ...
        for contest in self.future_contests:
            self.start_time_map[time.mktime(contest.start_time.timetuple())].append(
                contest
            )
    ...
    def _generate_contest_cache(self):
        ...
        self.contest_cache = [
            contest for contest in contests if contest.is_desired(website_schema.schema)
        ]
    ...
    def _reschedule_reminder_tasks(self, guild_id):
        ...
        channel, role = (
            guild.get_channel(settings.remind_channel_id),
            guild.get_role(settings.remind_role_id),
        )
        ...
        for start_time, contests in self.start_time_map.items():
            ...
            for contest in contests:
                website_seggregated_contests[contest.url] = (
                    contest  # an url can uniquely identify a contest
                )
        ...
        for data in pending_reschedule:
            ...
            if reaction_role is not None:
                task = asyncio.create_task(
                    self.send_finalcall_reminder(
                        embed, guild_id, reaction_role, send_time, link
                    )
                )
                self.finalcall_map[guild_id][link] = FinalCallRequest(
                    role_id=reaction_role.id, embed=embed, msg_id=data.msg_id
                )
    ...
    def _backup_serialize_guild_map(self):
        ...
        if (
            current_time_stamp - self.last_guild_backup_time
        ...
        out_path = Path(
            constants.GUILD_SETTINGS_MAP_PATH + "_" + str(current_time_stamp)
        )
        data = {"guild_map": self.guild_map, "finalcall_map": self.finalcall_map}
        with out_path.open(mode="wb") as out_file:
            pickle.dump(data, out_file)
    ...
    async def set_remind_settings(self, ctx, role: discord.Role, *before: int):
        ...
        remind_channel = ctx.guild.get_channel(
            self.guild_map[ctx.guild.id].remind_channel_id
        )
        remind_role = ctx.guild.get_role(self.guild_map[ctx.guild.id].remind_role_id)
        remind_before_str = (
            f"At {', '.join(str(mins) for mins in self.guild_map[ctx.guild.id].remind_before)} "
    ...
    async def reset_subscriptions(self, ctx):
        ...
        self.guild_map[ctx.guild.id].website_patterns = copy.deepcopy(
            website_schema.schema
        )
        await ctx.send(
            embed=discord_common.embed_success(
                "Succesfully reset the subscriptions to the default ones"
            )
        )
    ...
    def _set_guild_setting(self, guild_id, websites, unsubscribe):
        ...
        for website in websites:
            ...
            guild_settings.website_patterns[website].allowed_patterns = (
                []
                if unsubscribe
                else copy.deepcopy(website_schema.schema[website].allowed_patterns)
            )
            guild_settings.website_patterns[website].disallowed_patterns = (
                [""]
                if unsubscribe
                else copy.deepcopy(website_schema.schema[website].disallowed_patterns)
            )
    ...
    async def future(self, ctx, *filters):
        ...
        contests = filter_contests(
            filters, self.get_guild_contests(self.future_contests, ctx.guild.id)
        )
    ...
    async def active(self, ctx, *filters):
        ...
        contests = filter_contests(
            filters, self.get_guild_contests(self.active_contests, ctx.guild.id)
        )
        await self._send_contest_list(
            ctx,
            contests,
    ...
    async def finished(self, ctx, *filters):
        ...
        contests = filter_contests(
            filters, self.get_guild_contests(self.finished_contests, ctx.guild.id)
        )
        await self._send_contest_list(
            ctx,
            contests,
    ...
    async def get_finalcall_taskrole(self, guild_id, embed, remove=False):
        ...
        if link in self.finalcall_map[guild_id]:
            ...
            self.finalcall_map[guild_id][link] = FinalCallRequest(
                embed=embed, role_id=reaction_role.id
            )
            self.finaltasks[guild_id][link] = task
        else:
            reaction_role = None
    ...
    async def do_validation_check(self, payload):
        ...
        if (
            member.bot
            or settings.remind_channel_id is None
            or settings.remind_channel_id != payload.channel_id
            or payload.emoji.name != self.reaction_emoji
            or settings.finalcall_channel_id is None
        ):
            return None
        ...
        reaction_count = sum(
            ...
            if str(reaction) == self.reaction_emoji
        )
    ...
    async def on_raw_reaction_add(self, payload):
        ...
        self._serialize_guild_map()
        try:
            await member_dm.send(
                f"Final Call Alarm Set. You are alloted `{reaction_role.name}` which will be pinged"
    ...
    @commands.group(
        brief="Manage reactions in case of no reacts", invoke_without_command=True
    )
    ...
    @commands.group(
        brief="Manage reactions for self first react`", invoke_without_command=True
    )
    @commands.has_any_role("Admin", constants.REMIND_MODERATOR_ROLE)
    ...
    async def on_message(self, message):
        ...
        remind_role = self.bot.get_guild(message.guild.id).get_role(
            settings.remind_role_id
        )
    ...
    async def set_finalcall_settings(self, ctx, before: int):
        ...
        finalcall_channel = ctx.guild.get_channel(
            self.guild_map[ctx.guild.id].finalcall_channel_id
        )
    ...
    async def settings(self, ctx):
        ...
        if settings.remind_before is not None:
            remind_before_str = (
                f"At {', '.join(str(before_mins) for before_mins in settings.remind_before)}"
                f" mins before contest"
            )
        ...
        embed.add_field(
            name="Subscribed websites", value=f"{subscribed_websites_str}", inline=False
        )
